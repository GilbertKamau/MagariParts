    ...
    <header className='h-16 shadow-md bg-white'>
      <div className='h-full container mx-auto flex items-center px-4 justify-between'>
        ...
        <div className='flex items-center gap-7'>
          ...
          <div>
            {user?._id ? (
              <button onClick={handleLogout} className='px-3 py-1 rounded-full bg-orange-200 text-white hover:bg-orange-400'>Logout</button>
            ) : (
              <Link to={'/login'} className='px-3 py-1 rounded-full bg-orange-200 text-white hover:bg-orange-400'>Login</Link>
            )}
          </div>
        </div>
      </div>
